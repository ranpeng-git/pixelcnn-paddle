...
class PixelCNN(nn.Layer):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10, 
        ...
        self.right_shift_pad = nn.Pad2D([1, 0, 0, 0])
        self.down_shift_pad  = nn.Pad2D([0, 0, 1, 0])

        down_nr_resnet = [nr_resnet] + [nr_resnet + 1] * 2
        self.down_layers = nn.LayerList([PixelCNNLayer_down(down_nr_resnet[i], nr_filters, 
                                                self.resnet_nonlinearity) for i in range(3)])

        self.up_layers   = nn.LayerList([PixelCNNLayer_up(nr_resnet, nr_filters, 
                                                self.resnet_nonlinearity) for _ in range(3)])

        self.downsize_u_stream  = nn.LayerList([down_shifted_conv2d(nr_filters, nr_filters, 
                                                    stride=(2,2)) for _ in range(2)])

        self.downsize_ul_stream = nn.LayerList([down_right_shifted_conv2d(nr_filters, 
                                                    nr_filters, stride=(2,2)) for _ in range(2)])
        
        self.upsize_u_stream  = nn.LayerList([down_shifted_deconv2d(nr_filters, nr_filters, 
                                                    stride=(2,2)) for _ in range(2)])
        
        self.upsize_ul_stream = nn.LayerList([down_right_shifted_deconv2d(nr_filters, 
                                                    nr_filters, stride=(2,2)) for _ in range(2)])
        
        self.u_init = down_shifted_conv2d(input_channels + 1, nr_filters, filter_size=(2,3), 
                        shift_output_down=True)

        self.ul_init = nn.LayerList([down_shifted_conv2d(input_channels + 1, nr_filters, 
    ...
    def forward(self, x, sample=False):
        ...
        print(u_list) ### 这项输出越来越大，查看u_init对应函数 ，发现是conv之后的值变大

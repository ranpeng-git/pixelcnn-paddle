...
if args.load_params:
    ...
    # model.load_state_dict(paddle.load(args.load_params))
    print('model parameters loaded')


scheduler = paddle.optimizer.lr.StepDecay(learning_rate=args.lr, step_size=1, gamma=args.lr_decay)
optimizer = optim.Adam(learning_rate=0.0001, parameters=model.parameters())
...
for epoch in range(args.max_epochs):
    ...
    for batch_idx, (input,_) in enumerate(train_loader):
        ...
        # paddle.create_parameter(shape=input.shape,
        #                 dtype=str(input.numpy().dtype),
        #                 default_initializer=paddle.nn.initializer.Assign(input))
        output = model(input)
        
        # output = paddle.clip(output,min = -1,max = 1)
        # print("batch idx:",batch_idx,output[0,0,0,:])
